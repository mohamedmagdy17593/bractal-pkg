...
import generateSequenceFromTo from '~/core/utils/jsHelpers/generateSequence'
...
const modesColors = (type, theme, props) => {
  ...
  return {
    ...
    inverted: {
      ...
      borderColor: props.forceInvertedBorder
...
export const infereFontColor = (props, defaultColor) => {
  ...
  color =
    color || FONT_COLORS.find(fontColor => props[fontColor]) || defaultColor
...
export const infereFontSize = (props, size) => {
  ...
  if (_.isNumber(receivedSize)) {
    ...
    receivedSize &&
    parseFloat(receivedSize.toString()) === receivedSize
  ) {
...
export const responsiveStyle = (props, targetProp, callBack, defaultValue) => {
  ...
  if (_.isArray(propValue) && propValue.length >= 2) {
    assert(
      props.media,
      "Media isn't found. Make sure to surround with withMedia",
    )
    const currentSizeIndex = generateSequenceFromTo(4, -1).find(
      i => responsiveJSQueries(props.media)[i] && i < propValue.length,
    )
